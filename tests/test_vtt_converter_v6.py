...
from youtube_translator_desktop.services.vtt_converter_v6 import VttConverter


def test_vtt_converter_v6_better_separates_host_and_guest_turns() -> None:
    tmp_path = Path(".pytest_local") / f"vtt_v6_{uuid4().hex}"
    tmp_path.mkdir(parents=True, exist_ok=True)
    ko_vtt = tmp_path / "speaker.ko.vtt"
    en_vtt = tmp_path / "speaker.en.vtt"
    ko_vtt.write_text(
        """WEBVTT
...
""",
    ...
    document = converter.convert(
        DownloadResult(
            metadata=VideoMetadata(
                video_id="speakerv6",
                url="https://www.youtube.com/watch?v=speakerv6",
                title="화자 구분 테스트",
    ...
    first_host = lines.index("**진행자**")
    first_guest = lines.index("**게스트**")
    second_host = lines.index("**진행자**", first_guest + 1)
    second_guest = lines.index("**게스트**", second_host + 1)

    assert first_host < first_guest < second_host < second_guest
    assert "최근 GitHub 다운로드 실패가 자주 보였는지 궁금합니다." in document.markdown_text
    assert "에이전트 워크로드와 강화학습 루프가 길어지면서 CPU 요청이 급격히 늘어났습니다." in document.markdown_text
