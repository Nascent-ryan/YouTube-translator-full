...
def test_vtt_converter_v11_replaces_sentence_fragment_titles_with_stable_labels() -> None:
    tmp_path = Path(".pytest_local") / f"vtt_v11_{uuid4().hex}"
...
""",
    ...
    document = converter.convert(
        DownloadResult(
            metadata=VideoMetadata(
                video_id="awkwardv11",
                url="https://www.youtube.com/watch?v=awkwardv11",
                title="제목 품질 테스트",
    ...
    assert all("해보자" not in line for line in title_lines)
