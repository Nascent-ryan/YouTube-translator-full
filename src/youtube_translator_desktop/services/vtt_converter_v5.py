...
KEYWORD_PATTERN = re.compile(r"[가-힣A-Za-z0-9-]{3,}")
PARTICLE_SUFFIXES = (
    "에서는",
    "으로는",
    "에게는",
...
"""
    ...
    def _make_topic_title(self, turns: list[DialogueTurn]) -> str:
        labels: list[str] = []
        seen: set[str] = set()
        tokens = self._title_tokens(" ".join(turn.text for turn in turns))
        for token in tokens:
        ...
        if labels:
            labels.sort(key=lambda item: (-TITLE_PRIORITIES.get(item, 0), item))
            if len(labels) == 1:
                return labels[0]
            return f"{labels[0]}{self._topic_joiner(labels[0])}{labels[1]}"

        keywords = self._keywords(" ".join(turn.text for turn in turns))
        ordered = sorted((self._display_keyword(word) for word in keywords), key=lambda item: (-len(item), item))
        if not ordered:
            return "주요 논의"
        return ordered[0]
    ...
    def _title_tokens(self, text: str) -> list[str]:
        tokens: list[str] = []
        for raw_word in KEYWORD_PATTERN.findall(text.lower()):
            cleaned = self._strip_particle(raw_word)
            if len(cleaned) < 3 or cleaned in STOPWORDS:
                continue
            tokens.append(cleaned)
        return tokens

    def _keyword_to_title_label(self, token: str) -> str | None:
        if token in TITLE_LABELS:
            return TITLE_LABELS[token]
        if token.endswith("github"):
            return "GitHub 안정성"
        if token.endswith("microsoft"):
            return "마이크로소프트 전략"
    ...
    def _display_keyword(self, keyword: str) -> str:
        if keyword.isascii():
            return keyword.upper() if len(keyword) <= 4 else keyword.capitalize()
        return keyword

    def _topic_joiner(self, text: str) -> str:
        last_char = text[-1]
    ...
    def _strip_particle(self, word: str) -> str:
    ...
    def _keywords(self, text: str) -> set[str]:
        words = {self._strip_particle(match.group(0).lower()) for match in KEYWORD_PATTERN.finditer(text)}
